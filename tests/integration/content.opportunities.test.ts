...
describe('P5-A content opportunities', () => {
  ...
  it('opens deterministic failures and only deterministic PASS verifies them fixed', async () => {
    ...
    const snapshot = await prisma.pageSnapshot.create({ data: { pageId: page.id, crawlRunId: crawl.id, finalUrl: page.url, statusCode: 200, contentType: 'text/html', title: '', h1: '', wordCount: 100, h1Count: 1, h2Count: 0, h3Count: 0, internalLinksCount: 0, contentHash: `h-${suffix}`, parserVersion: 'test' } });

    const facts = buildContentFacts({ projectId: project.id, pageId: page.id, normalizedUrl: page.normalizedUrl, snapshotId: snapshot.id, statusCode: 200, contentType: 'text/html', canonicalUrl: null, title: '', metaDescription: '', h1: '', language: null, wordCount: 100, h1Count: 1, h2Count: 0, h3Count: 0, imagesCount: 0, internalLinksCount: 0, externalLinksCount: 0, schemaTypes: [], contentHash: snapshot.contentHash, capturedAt: snapshot.capturedAt });
    const document = await contentRepository.upsertContentDocument(facts);
    await contentRepository.replaceEvaluation(project.id, document.id, evaluateContentDocument(facts, { entityCount: 0, citabilityScore: 20, schemaTypesKnown: true }));
    ...
    await contentRepository.replaceEvaluation(project.id, document.id, evaluateContentDocument(healthy, { entityCount: 2, citabilityScore: 80, schemaTypesKnown: true }));
