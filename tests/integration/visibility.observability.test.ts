...
describe('P6-A visibility observability', () => {
  ...
  async function createRunFixture(label: string, promptTexts: string[]) {
    ...
    const project = await prisma.project.create({
      data: {
        name: `Visibility Observability ${label}`,
    ...
    const promptSet = await prisma.visibilityPromptSet.create({
      data: { projectId: project.id, name: `Observability ${label}` }
    });
    for (const [index, promptText] of promptTexts.entries()) {
      await prisma.visibilityPrompt.create({
        data: {
          ...
          promptKey: `prompt-${index + 1}`,
          version: 1,
          promptText,
          promptHash: `hash-${suffix}-${index}`
        }
      });
    }
    ...
    const run = await new VisibilityRunService(queue).createManualRun(project.id, {
      promptSetId: promptSet.id,
      providerConfigIds: [provider.id],
      maxObservations: promptTexts.length,
      budgetCeilingMicros: 10_000
    });
  ...
  it('wires run and worker success events without logging prompt or answer bodies', async () => {
    ...
    const registry = new VisibilityProviderRegistry([new FixtureAdapter()]);
  ...
  it('emits unsupported and failed final run events', async () => {
    ...
    const registry = new VisibilityProviderRegistry([new FixtureAdapter(false)]);
  ...
  it('emits observation failure and run partial when one of two samples fails', async () => {
    ...
    await executeVisibilityObservation(queue.observationIds[0]!, { registry });
    await expect(executeVisibilityObservation(queue.observationIds[1]!, { registry })).rejects.toMatchObject({
      code: 'VISIBILITY_PROVIDER_FAILED'
    });
