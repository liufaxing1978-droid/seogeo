...
describe('P6-A visibility lifecycle observability', () => {
  ...
  it('emits run and observation lifecycle events without prompt/answer bodies', async () => {
    ...
    const project = await prisma.project.create({ data: { name: 'Visibility Observability Lifecycle', slug: `visibility-observability-${suffix}`, primaryDomain: `visibility-observability-${suffix}.example.com`, planLevel: 'ADVANCED' } });
    projectIds.push(project.id);
    await prisma.visibilityProjectSettings.create({ data: { projectId: project.id, dailyBudgetMicros: 2_000_000, defaultRunBudgetMicros: 500_000, maxObservationsPerRun: 10, defaultCurrency: 'USD' } });
    const set = await prisma.visibilityPromptSet.create({ data: { projectId: project.id, name: 'Observability set' } });
    const prompt = await prisma.visibilityPrompt.create({ data: { projectId: project.id, promptSetId: set.id, promptKey: 'observability', version: 1, promptText: 'Private prompt body must never be logged.', promptHash: `hash-${suffix}` } });
    ...
    const run = await service.createManualRun(project.id, { promptSetId: set.id, providerConfigIds: [provider.id], maxObservations: 1, budgetCeilingMicros: 100_000 });
    const observation = await prisma.platformObservation.findFirstOrThrow({ where: { visibilityRunId: run.id } });
    await executeVisibilityObservation(observation.id, { registry: new VisibilityProviderRegistry([new FixtureAdapter()]) });
    ...
    expect(emitted).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ event: 'visibility.observation.started', projectId: project.id, runId: run.id, observationId: observation.id, provider: 'OPENAI', model: 'gpt-5-mini', channel: 'API', promptId: prompt.id, promptVersion: 1 }),
      expect.objectContaining({ event: 'visibility.observation.completed', observationId: observation.id, status: 'COMPLETED', totalTokens: 30, searchUnits: 1, costMicros: 1200 }),
