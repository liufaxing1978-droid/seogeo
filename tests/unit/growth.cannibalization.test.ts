...
function candidate(
  ...
  const queryImpressions = pages.reduce((sum, row) => sum + row.impressions, 0);
  return {
    normalizedQuery: 'q',
    demandScore: 65,
    queryImpressions,
    projectP50Impressions: 20,
    pages,
    ...overrides
  };
}
...
describe('P7-A keyword cannibalization detector', () => {
  it('detects balanced material pages when demand, share and ranking competition are satisfied', () => {
    const result = detectKeywordCannibalization(candidate([
  ...
  it('requires Demand Score >=40, total Query impressions >=20 and Query-level project P50', () => {
    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 50, 8), page('https://e.com/b', 50, 9)
    ], { demandScore: 39 })).state).toBe('NOT_DETECTED');
  ...
  it('requires two pages each >=20% while no page reaches 80%', () => {
    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 80, 8), page('https://e.com/b', 20, 9)
    ])).state).toBe('NOT_DETECTED');

    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 81, 8), page('https://e.com/b', 19, 9)
    ])).state).toBe('NOT_DETECTED');
  });

  it('requires ranking competition: both material pages top 30 or position distance <=10', () => {
    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 55, 25), page('https://e.com/b', 45, 30)
    ])).state).toBe('DETECTED');

    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 55, 35), page('https://e.com/b', 45, 44)
    ])).state).toBe('DETECTED');

    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 55, 35), page('https://e.com/b', 45, 46)
    ])).state).toBe('NOT_DETECTED');
  });
  ...
  it('uses share, position, CTR, then evidence strength for the primary-page candidate', () => {
    const share = detectKeywordCannibalization(candidate([
      page('https://e.com/a', 55, 20, 0.02), page('https://e.com/b', 45, 5, 0.20)
    ]));
    expect(share.primaryPageCandidate).toEqual({ state: 'KNOWN', canonicalPage: 'https://e.com/a' });

    const position = detectKeywordCannibalization(candidate([
      page('https://e.com/a', 50, 8, 0.02), page('https://e.com/b', 50, 10, 0.20)
    ]));
    expect(position.primaryPageCandidate).toEqual({ state: 'KNOWN', canonicalPage: 'https://e.com/a' });

    const ctr = detectKeywordCannibalization(candidate([
      page('https://e.com/a', 50, 8, 0.10), page('https://e.com/b', 50, 8, 0.08)
    ]));
    expect(ctr.primaryPageCandidate).toEqual({ state: 'KNOWN', canonicalPage: 'https://e.com/a' });

    const evidence = detectKeywordCannibalization(candidate([
      page('https://e.com/a', 50, 8, 0.10), page('https://e.com/b', 50, 8, 0.10)
  ...
  it('fails closed on missing demand floors/ranking evidence and enforces the 20-page bound', () => {
    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 50, 8), page('https://e.com/b', 50, 9)
    ], { demandScore: null })).state).toBe('UNKNOWN');
    ...
    expect(detectKeywordCannibalization(candidate([
      page('https://e.com/a', 50, null), page('https://e.com/b', 50, 9)
    ])).state).toBe('UNKNOWN');

    expect(() => detectKeywordCannibalization(candidate(
      Array.from({ length: 21 }, (_, index) => page(`https://e.com/${index}`, 10, 10))
    ))).toThrow(/20/);
  });
});