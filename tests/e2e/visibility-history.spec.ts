...
test('navigates P6-D history and alerts without triggering sampling', async ({ page, context }) => {
  ...
  await expect(visibilitySubnav.getByRole('link', { name: '历史', exact: true })).toHaveAttribute('aria-current', 'page');
  await visibilitySubnav.getByRole('link', { name: '告警', exact: true }).click();
  ...
  await expect(page.getByRole('navigation').getByRole('link', { name: 'GEO / 可见度', exact: true })).toHaveAttribute('aria-current', 'page');
  await expect(page.getByLabel('GEO 与 AI Visibility 二级导航').getByRole('link', { name: '告警', exact: true })).toHaveAttribute('aria-current', 'page');
  ...
  await page.getByLabel('GEO 与 AI Visibility 二级导航').getByRole('link', { name: 'AI Visibility', exact: true }).click();
