...
test('generates and opens a project report without invoking DeepSeek', async ({ page }) => {
  ...
  await expect(page.getByText('PROJECT_REPORT_V1')).toBeVisible();
  await page.getByRole('link', { name: '查看报告' }).first().click();
  await expect(page.getByText('确定性事实')).toBeVisible();
  await expect(page.getByText('AI 建议 / Executive Summary')).toBeVisible();
