...
export interface PerplexityVisibilityHttpRequest {
  url: string;
  method: 'POST';
...
export interface PerplexityVisibilityHttpResponse {
  status: number;
  body: unknown;
  latencyMs: number;
}

export interface PerplexityVisibilityTransport {
  send(request: PerplexityVisibilityHttpRequest): Promise<PerplexityVisibilityHttpResponse>;
}
...
function normalizeUsage(value: unknown) {
  ...
  return {
    ...
    searchUnits: nonNegativeInteger(usage?.num_search_queries)
  };
}
...
function normalizeReportedCost(value: unknown) {
  ...
  const totalCostUsd = nonNegativeNumber(cost?.total_cost);
  ...
  return {
    costMicros: Math.round(totalCostUsd * 1_000_000),
    costCurrency: 'USD',
    pricingVersion: PERPLEXITY_REPORTED_COST_VERSION
  };
}
...
function normalizeSearchResults(value: unknown): SafeSearchResult[] {
  ...
  for (const item of value) {
    ...
    results.push({
      ...
      url,
      date: stringValue(itemRecord?.date),
      lastUpdated: stringValue(itemRecord?.last_updated),
      source: stringValue(itemRecord?.source)
    });
  }
  return results;
}
...
function providerHttpError(status: number): VisibilityProviderError {
  if (status === 401 || status === 403) {
    return new VisibilityProviderError(
      ...
      `Perplexity visibility request failed with HTTP ${status}`,
  ...
  if (status === 429) {
    return new VisibilityProviderError(
      ...
      'Perplexity visibility request was rate limited',
      { httpStatus: status, retryable: false }
    );
  }
  return new VisibilityProviderError(
...
export class PerplexityVisibilityProvider implements VisibilityProviderAdapter {
  ...
  async sample(request: VisibilitySampleRequest): Promise<VisibilitySampleResponse> {
    ...
    try {
      ...
      throw new VisibilityProviderError(
        'VISIBILITY_PROVIDER_FAILED',
        'Perplexity visibility request failed',
        { retryable: false }
      );
    }
    ...
    if (!responseBody || !id || choices === null) {
      throw new VisibilityProviderError(
        'VISIBILITY_PROVIDER_MALFORMED_RESPONSE',
        'Perplexity returned a malformed visibility response',
