...
A P9-D experiment may contain multiple planned windows, for example 7/14/28 or 14/28/56 days. A single terminal window may also have more than one immutable observation because P9-D observation identity includes cutoff/source references. Counting all windows or selecting a favorable later observation would overweight or cherry-pick one intervention.

P9-E therefore accepts at most one observation per experiment.

### 5.1 Terminal schedule

The terminal schedule entry is the final server-derived item in the experiment's frozen `observationScheduleJson`. P9-D V1 schedules are ordered ascending by window duration, so this is the longest planned window.

A terminal candidate observation must:

- belong to the exact experiment;
- match the terminal scheduled `windowType` and `windowDays`;
- have `dueAt` equal to the frozen terminal due time;
- use a supported P9-D evaluator version;
- have a unique immutable P9-D observation identity.

Earlier planned windows can never create P9-E evidence.

### 5.2 Multiple terminal observations

When multiple immutable P9-D observations exist for the terminal schedule, P9-E evaluates them in deterministic order:

1. `inputCutoffAt` ascending;
2. `observationId` ascending as the tie breaker.

The first terminal observation that satisfies every feedback-eligibility gate in section 6 becomes the sole accepted evidence for the experiment.

An earlier terminal observation that is `INCONCLUSIVE`, insufficient, or contaminated does not become a negative sample and does not permanently poison the experiment; a later terminal observation may qualify. Once one observation is accepted, every later observation for that experiment is ignored for feedback forever.

This prevents both double counting and outcome cherry-picking.

The accepted-evidence table enforces uniqueness by both `observationId` and `experimentId`.

## 6. Feedback eligibility

A terminal observation contributes only when all conditions are true:

1. it matches the terminal schedule contract in section 5;
...
- idempotent create-or-get compares the full immutable payload and fails closed on mismatch.
...
A deterministic evidence key is SHA-256 over canonical JSON containing exactly:
...
6. calculate `inputFingerprint` from `feedbackProfileVersion + scopeKey + ordered evidence IDs`;
7. calculate `profileKey` from `feedbackProfileVersion + projectId + scopeKey + inputFingerprint`.
...
P9-E implementation introduces planner version:

```text
OPTIMIZATION_PLAN_V2
```

When P9-A V2 freezes a new plan, it reads the latest compatible feedback profile for the exact candidate scope and deterministic recommended action.
...
If application of historical feedback would cause any candidate to exceed this displacement bound, the planner retries that materialization with all historical adjustments set to zero while preserving the already-valid AI adjustment contract. The fallback is recorded in the frozen explanation.
...
Primary handoff may be emitted after P9-D persists any observation. The P9-E worker itself determines whether that observation belongs to the terminal schedule and whether the experiment now has an acceptable terminal observation under sections 5 and 6.
...
Daily reconciliation is fixed in V1 to:

- scan terminal P9-D observations with `createdAt` in the previous 90 days only;
- process at most 100 candidate observations per project per run;
- use queue `attempts = 2`;
- make zero paid/external provider, DeepSeek, Git, publication, deployment, or rollback calls;
- skip experiments that already have accepted feedback evidence.

V1 has no unlimited or public manual historical backfill. A future backfill requires a separate reviewed design/change.

## 12. API and UI boundary

P9-E V1 exposes exactly these project-scoped persisted-read routes:
...
P9-E V1 exposes no public POST materialization or reconciliation endpoint. Queue handoff and bounded reconciliation are internal worker paths.
...
P9-E introduces the explicit feature capability:
...
Feature denial occurs before restricted repository reads, queue operations, or mutation side effects.

`OPTIMIZATION_FEEDBACK` availability does not alter P9-C controlled-autopilot policy or enable any new execution authority.
...
- multiple terminal observations choose the earliest fully eligible observation by cutoff/id;
- an earlier inconclusive terminal observation can be followed by one later eligible accepted observation;
- once accepted, later terminal observations cannot contribute;
...
- reconciliation scans no more than 90 days and 100 candidates/project/run;
...
1. add P9-E feature capability, enums/tables/triggers/indexes;
2. add terminal selection, eligibility, immutable evidence/profile materialization;
3. add bounded queue handoff/reconciliation and observability;
4. verify profile calculation independently before planner consumption;
5. introduce P9-A `OPTIMIZATION_PLAN_V2` consumption only after feedback persistence/identity tests are stable;
6. add persisted-read API and bounded audit projection;
7. keep all P9-A V1 and P9-D historical records untouched.

The P9-E branch is intentionally stacked on the P9-D exact head while PR #158 remains unmerged. P9-E must not cause P9-D PR #158 to receive new commits. Integration/retargeting to `main` occurs only after the P9-D dependency is separately merged/authorized.
...
- public/manual unlimited historical backfill.
...
- deterministic selection of the earliest fully eligible terminal observation;
